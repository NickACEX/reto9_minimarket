...
class Connection:
    ...
    def get_by_id_column(self, id_object, id_column):
        query = f'''
            SELECT {id_column} FROM {self.table_name} WHERE 
            {"".join(map(str, id_object.keys()))} = {"".join(map(str, id_object.values()))}
        '''
        self.cursor.execute(query)
    ...
    def get_by_id_column_top(self, id_object):
        query = f'''
            SELECT {id_object} FROM {self.table_name} ORDER BY {id_object} DESC LIMIT 1
        '''
        self.cursor.execute(query)
        result=self.cursor.fetchone()
        return result

    def get_by_login(self, id_user, id_pass):
        query = f'''
            SELECT perfil_id FROM {self.table_name} WHERE 
            usuario = '{"".join(map(str, id_user.values()))}' AND
            contraseña = '{"".join(map(str, id_pass.values()))}'
        '''
        self.cursor.execute(query)
        result=self.cursor.fetchone()
        return result
